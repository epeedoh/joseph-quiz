...
namespace JosephQuiz.Application.Features.Scores.Commands.SubmitScore;

public sealed record SubmitScoreCommand(ScoreSubmissionRequest Payload) : IRequest<QuizResultDto>;
...
public sealed class SubmitScoreCommandValidator : AbstractValidator<SubmitScoreCommand>
{
    public SubmitScoreCommandValidator()
    {
        ...
        RuleForEach(command => command.Payload.Answers).ChildRules(answer =>
        {
            answer.RuleFor(item => item.SelectedOption).NotEmpty().MaximumLength(1);
            answer.RuleFor(item => item.ResponseTimeMs).GreaterThanOrEqualTo(0);
        });
    }
}
...
public sealed class SubmitScoreCommandHandler(
    ...
    IUnitOfWork unitOfWork) : IRequestHandler<SubmitScoreCommand, QuizResultDto>
{
    public async Task<QuizResultDto> Handle(SubmitScoreCommand request, CancellationToken cancellationToken)
    {
        ...
        var evaluation = QuizScoringEngine.Evaluate(questions, request.Payload.Answers, QuizRules.FastAnswerThresholdMs);
        ...
        await scoreRepository.AddAsync(new JosephQuiz.Domain.Entities.Score
        {
            ...
            Mode = JosephQuiz.Domain.Enums.QuizMode.Revision,
            Zone = zone,
