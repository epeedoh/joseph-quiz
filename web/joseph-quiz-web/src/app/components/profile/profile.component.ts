  ...
  template: `
    @if (!progressService.profile()) {
      <section class="glass-card space-y-5 p-8 text-center">
        ...
        <p class="text-sm text-ink/70">Lance un premier quiz pour voir apparaître ton score, ta precision et tes recommandations.</p>
        <a routerLink="/" class="mx-auto inline-flex rounded-full bg-royal px-5 py-3 text-sm font-extrabold uppercase tracking-[0.18em] text-white">Demarrer</a>
      </section>
    } @else {
      <section class="space-y-6">
        <div class="grid gap-6 xl:grid-cols-[0.62fr_0.38fr]">
          <article class="glass-card p-6 sm:p-8">
            ...
            <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <div class="rounded-[24px] bg-ink/5 p-4">
                <p class="text-xs uppercase tracking-[0.18em] text-ink/45">Precision</p>
                <p class="mt-2 text-2xl font-extrabold text-ink">{{ ((progressService.profile()?.accuracy ?? 0) * 100) | number:'1.0-0' }}%</p>
              </div>
              <div class="rounded-[24px] bg-ink/5 p-4">
                <p class="text-xs uppercase tracking-[0.18em] text-ink/45">Temps moyen</p>
                <p class="mt-2 text-2xl font-extrabold text-ink">{{ progressService.profile()?.averageResponseTimeMs | number:'1.0-0' }} ms</p>
              </div>
              <div class="rounded-[24px] bg-ink/5 p-4">
                <p class="text-xs uppercase tracking-[0.18em] text-ink/45">Equipe</p>
                <p class="mt-2 text-2xl font-extrabold text-ink">{{ progressService.profile()?.teamName || 'Aucune' }}</p>
              </div>
            </div>
          </article>

          <aside class="glass-card p-6">
            <p class="gold-chip">Synthese</p>
            <h2 class="mt-4 font-display text-2xl text-royal">Lecture rapide</h2>
            <div class="mt-6 space-y-4 text-sm text-ink/70">
              <p>Equipe actuelle: <span class="font-bold text-ink">{{ progressService.profile()?.teamName || 'Aucune' }}</span></p>
              <p>Temps de reponse moyen: <span class="font-bold text-ink">{{ progressService.profile()?.averageResponseTimeMs | number:'1.0-0' }} ms</span></p>
              <p>Le tableau de bord met l'accent sur l'essentiel: score, precision et historique recent.</p>
            </div>
          </aside>
        </div>
        ...
        <section class="grid gap-6 xl:grid-cols-[0.56fr_0.44fr]">
          ...
          <article class="glass-card p-6">
            ...
            <h2 class="mt-4 section-title">Dernieres sessions</h2>
            <div class="mt-6 space-y-4">
              @for (score of progressService.profile()?.recentScores ?? []; track score.id) {
                <div class="rounded-[24px] bg-ink/5 p-4">
                  ...
                  <div class="mt-3 flex items-center justify-between text-sm text-ink/70">
                    <span>{{ score.correctAnswers }}/{{ score.totalQuestions }} bonnes reponses</span>
